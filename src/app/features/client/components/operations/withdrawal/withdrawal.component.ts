        ...
        <form [formGroup]="withdrawalForm" (ngSubmit)="onSubmit()">
          ...
          <div class="form-group">
            ...
            <textarea 
              ...
              class="form-control"
              rows="3"
              placeholder="Ex: Retrait pour achat immobilier"
            ></textarea>
          </div>
          ...
          <div class="form-actions">
            ...
            >
              ...
              <span *ngIf="!isLoading">Effectuer le retrait</span>
            </button>
          </div>
        </form>
      </div>
...
export class WithdrawalComponent implements OnInit {
    ...
    get requiresDocument(): boolean {
        const montant = this.withdrawalForm?.get('montant')?.value;
    ...
    uploadDocument(operationId: number): void {
        ...
        this.documentService.uploadDocument(operationId, this.selectedFile).subscribe({
            ...
            error: (error) => {
                this.isLoading = false;
                this.showError = true;
                this.errorMessage = 'Retrait créé mais erreur lors de l\'upload du justificatif';
                console.error(error);
            }
        });
    }
    ...
    handleSuccess(): void {
        ...
        this.successMessage = this.requiresDocument
            ? 'Retrait créé avec succès. Votre opération est en attente de validation.'
    ...
    getErrorMessage(error: any): string {
        if (error.status === 400) {
            return error.error?.message || 'Solde insuffisant ou données invalides';
        }
        return 'Une erreur est survenue lors de la création du retrait';
    }

    isFieldInvalid(fieldName: string): boolean {
        const field = this.withdrawalForm.get(fieldName);
    ...
    getFieldError(fieldName: string): string {
        ...
        if (field.errors['maxAmount']) {
            return `Le montant ne peut pas dépasser ${field.errors['maxAmount'].max} DH`;
        }
